    ...
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        def get_median_from_array(numbers: List[int]) -> float:
            if total_size % 2 != 0:
                return numbers[
                    total_half_size - 1
                ]  # middle element idx = total_half_size - 1
        ...
        while left_num1_idx <= right_num1_idx:
            ...
            num1_max_left_elem: int = (
                nums1[middle_num1_idx] if middle_num1_idx >= 0 else -math.inf
            )
            num1_min_right_elem: int = (
                nums1[middle_num1_idx + 1]
                if middle_num1_idx + 1 < len(nums1)
                else math.inf
            )

            num2_max_left_elem: int = (
                nums2[left_num2_idx] if left_num2_idx >= 0 else -math.inf
            )
            num2_min_right_elem: int = (
                nums2[left_num2_idx + 1] if left_num2_idx + 1 < len(nums2) else math.inf
            )

            if (
                num1_max_left_elem <= num2_min_right_elem
                and num2_max_left_elem <= num1_min_right_elem
            ):
        ...
        num1_min_right_elem: int = (
            nums1[middle_num1_idx + 1] if middle_num1_idx + 1 < len(nums1) else math.inf
        )
